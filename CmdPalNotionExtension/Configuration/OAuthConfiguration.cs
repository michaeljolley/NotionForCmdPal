...
{
  // This redirect url has to be configured into the OAuth app. This package has "cmdpalnotionext://"
  // protocol extension and will receive the access token in this uri after successful login.
  internal const string RedirectUri = "cmdpalnotionext://oauth_redirect_uri/";
  ...
  {
    internal static readonly string ClientID = "%BUILD_TIME_NOTION_CLIENT_ID_PLACEHOLDER%";

    internal static readonly string ClientSecret = "%BUILD_TIME_NOTION_CLIENT_SECRET_PLACEHOLDER%";
  }

  public static string GetClientId()
  {
    if (BuildTimeReplacements.ClientID.Equals("%" + "BUILD_TIME_NOTION_CLIENT_ID_PLACEHOLDER" + "%", StringComparison.OrdinalIgnoreCase))
    {
  ...
  {
    if (BuildTimeReplacements.ClientSecret.Equals("%" + "BUILD_TIME_NOTION_CLIENT_SECRET_PLACEHOLDER" + "%", StringComparison.OrdinalIgnoreCase))
    {
      if (string.IsNullOrEmpty(DeveloperOAuthConfiguration.ClientSecret))
      {
        // Throw if neither the Build-time constant or the environment variable is set.
        throw new InvalidOperationException("Client Secret has not been set.");
      }
